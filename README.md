...
- ✅ **Yosys**：用于Formal验证
  - **版本要求**：≥ 0.56（重要！Yosys 0.9无法正确验证输出端口等价性）
  - 安装：
    - macOS：`brew install yosys`
    - Linux：
      - 如果apt版本≥0.56：`sudo apt-get install yosys`
      - 否则从源码安装：参考 `scripts/reinstall_yosys_from_source.sh`
  - 验证：`yosys -V`
  - **服务器安装**：
    - 自动安装脚本：`bash scripts/reinstall_yosys_from_source.sh`
    - 详细指南：`scripts/YOSYS_REINSTALL_GUIDE.md`
    - GitHub源码：https://github.com/YosysHQ/yosys
  - **注意**：如果服务器上Yosys版本过低，Formal验证可能失败。请使用重新安装脚本升级。
...
**Formal验证用于确保层级化改造后的网表与原始网表功能等价**。

**工作原理**：
- 使用Yosys进行等价性检查
- 比较flatten网表（原始）和hierarchical网表（分区后）的功能等价性
- 验证层级化改造的正确性

**关键问题与解决方案**：

1. **问题**：Yosys的`hierarchy -check`命令会严格检查所有模块定义
   - 标准单元在`cells.lef`中有物理定义，但在Verilog网表中没有逻辑定义
...
2. **解决方案**：去掉`hierarchy -check`中的`-check`选项
...
**详细分析文档**：
- `FORMAL_VERIFICATION_ANALYSIS.md`：完整的原因分析和解决方案
- `FORMAL_VERIFICATION_FIXED.md`：问题修复总结
...
K-SpecPart是一个**超图分区优化工具**，解决的核心问题是：

**问题定义**：
```
给定：
  - 超图 H = (V, E)
    • V：顶点集合（对应芯片设计中的components）
    • E：超边集合（对应nets，每个net可以连接多个components）
  - 分区数量 K（例如 K=4）
  - 平衡约束 ε（例如 ε=0.05，各分区大小差异≤5%）

目标：
  - 将 V 分成 K 个子集：V1, V2, ..., VK
  - 最小化 Cutsize（被切断的超边数量）
  - 满足平衡约束：|Vi| ≈ n/K
```

**关键点**：
- ✅ K-SpecPart提供**逻辑分区决策**（component → partition映射）
- ❌ **不生成分区后的netlist**
- ❌ **不生成物理布局**
- ❌ **不做Formal验证**

K-SpecPart的输出（`.part.4`文件）是"分区蓝图"，告诉我们"哪个component应该在哪个partition"，但**后续工作需要我们实现**。

### 完整的Partition-based Flow

基于K-SpecPart的分区决策，我们需要实现以下完整流程：

```
┌─────────────────────────────────────────────────────────────┐
│ 输入：design.v (flatten门级网表, 32281 components)          │
│       + LEF files                                           │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 1: K-SpecPart 超图分区                                 │
│   → .part.4（32281行，每行一个partition_id: 0-3）           │
│   → Cutsize=219（被切断的nets数量）                         │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 2: 生成分区网表（VerilogPartitioner）                  │
│   → partition_0.v ~ partition_3.v (各分区子网表)           │
│   → top.v (顶层网表，实例化4个partition模块)                │
│   → boundary_nets.json (边界nets分析)                       │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 3: Formal验证（Yosys）⭐ 关键                          │
│   验证：design.v ≈ top.v + partition_*.v                    │
│   → 确保层次化改造后功能等价                                 │
│   → ✅ 已验证通过（使用hierarchy而非hierarchy -check）      │
│   → 详细说明见"Formal验证说明"章节                          │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 4: 物理位置优化（已实现）                               │
│   → 基于boundary_nets的连接性优化partition布局              │
│   → physical_regions = {0: (x,y,w,h), ...}                 │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 5: 各Partition OpenROAD执行（并行）                    │
│   → partition_0_layout.def ~ partition_3_layout.def        │
...
│   → partition_0.lef ~ partition_3.lef                       │
│   → 将partition视为macro，用于顶层组装                      │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 7: 顶层OpenROAD执行（boundary nets only）              │
│   → top_layout.def (只布线boundary nets)                    │
│   → 提取boundary HPWL                                        │
└───────────────────┬─────────────────────────────────────────┘
                    │
                    ▼
...
**核心洞察**：
- **K-SpecPart**：是"决策者"（Logic-level）
- **VerilogPartitioner**：是"执行者"（Implementation-level），我们需要实现
- **Formal验证**：是"质量保证"（Correctness）
...
| **Yosys** | 0.56 | `/opt/homebrew/bin/yosys` | ✅ |
...
- 映射文件: `results/kspecpart/{design}/{design}.mapping.json`
...
**Cutsize含义**：219条nets被切断，这些是**boundary nets**（跨partition的nets）

### 待实现的核心模块

| 模块 | 文件 | 功能 | 依赖 | 优先级 | 时间 |
|------|------|------|------|--------|------|
| **VerilogPartitioner** | `src/utils/verilog_partitioner.py` | 基于K-SpecPart结果生成分区网表 | K-SpecPart输出 | P0 🔥 | 1天 |
| **Formal验证集成** | 已有`formal_verification.py` | 验证flatten ≈ hierarchical | VerilogPartitioner | P0 🔥 | 0.5天 |
| **Partition-based Flow** | `scripts/run_partition_based_flow.py` | 完整流程编排 | 上述所有 | P0 🔥 | 0.5天 |
...
| ILP partitioner CPLEX限制 | 禁用ILP，回退到hMETIS | `optimal_attempt_partitioner.jl` |
| TritonPart路径拼接 | 添加空格分隔 | `run_triton_part_refiner.jl:17` |
| Python找不到Julia | 自动搜索`~/.juliaup/bin/julia` | `run_kspecpart_experiment.py:76` |
| Formal验证hierarchy -check报错 | 去掉`-check`选项，允许标准单元作为黑盒 | `formal_verification.py:110,128` |

### 参考文档

- **K-SpecPart深度分析**：`KSPECPART_ANALYSIS.md`
- **完整计划**：`docs/chipmasrag.plan.md`
- **集成总结**：`KSPECPART_INTEGRATION_SUMMARY.md`
